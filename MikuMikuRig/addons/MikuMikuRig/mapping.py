...
def draw_panel(layout):
    ...
    if s.editing_type == 0:
        ...
        if s.selected_count == len(s.mappings):
            box_left.operator('mmr_kumopult_bac.select_action', text='', emboss=False, icon='CHECKBOX_HLT').action = 'NONE'
        else:
            box_left.operator('mmr_kumopult_bac.select_action', text='', emboss=False, icon='CHECKBOX_DEHLT').action = 'ALL'
            if s.selected_count != 0:
                # 反选按钮仅在选中部分时出现
                box_left.operator('mmr_kumopult_bac.select_action', text='', emboss=False,
                                  icon='UV_SYNC_SELECT').action = 'INVERSE'
    # 编辑模式切换
    box_right = box.row(align=False)
    box_right.alignment = 'RIGHT'
    box_right.operator('mmr_kumopult_bac.select_edit_type', text='' if s.editing_type != 0 else '映射', icon='PRESET',
                       emboss=True, depress=s.editing_type == 0).selected_type = 0
    box_right.operator('mmr_kumopult_bac.select_edit_type', text='' if s.editing_type != 1 else '旋转', icon='CON_ROTLIKE',
                       emboss=True, depress=s.editing_type == 1).selected_type = 1
    box_right.operator('mmr_kumopult_bac.select_edit_type', text='' if s.editing_type != 2 else '位移', icon='CON_LOCLIKE',
                       emboss=True, depress=s.editing_type == 2).selected_type = 2
    box_right.operator('mmr_kumopult_bac.select_edit_type', text='' if s.editing_type != 3 else 'ＩＫ', icon='CON_KINEMATIC',
                       emboss=True, depress=s.editing_type == 3).selected_type = 3
    # 映射列表
    left.template_list('MMR_BAC_UL_mappings', '', s, 'mappings', s, 'active_mapping', rows=7)
    # 预设菜单
    box = left.box().row(align=True)
    box.menu(MMR_BAC_MT_presets.__name__, text=MMR_BAC_MT_presets.bl_label, translate=False, icon='PRESET')
    box.operator(MMR_AddPresetBACMapping.bl_idname, text="", icon='ADD')
    box.operator(MMR_AddPresetBACMapping.bl_idname, text="", icon='REMOVE').remove_active = True
    box.separator()
    box.operator('mmr_kumopult_bac.open_preset_folder', text="", icon='FILE_FOLDER')

    right = row.column(align=True)
    right.separator()
    # 设置菜单
    right.menu(MMR_BAC_MT_SettingMenu.__name__, text='', icon='DOWNARROW_HLT')
    right.separator()
    # 列表操作按钮
    if s.owner.mode != 'POSE':
        right.operator('mmr_kumopult_bac.list_action', icon='ADD', text='').action = 'ADD'
    elif s.target.mode != 'POSE':
        right.operator('mmr_kumopult_bac.list_action', icon='PRESET_NEW', text='').action = 'ADD_SELECT'
    else:
        right.operator('mmr_kumopult_bac.list_action', icon='PLUS', text='').action = 'ADD_ACTIVE'
    right.operator('mmr_kumopult_bac.list_action', icon='REMOVE', text='').action = 'REMOVE'
    right.operator('mmr_kumopult_bac.list_action', icon='TRIA_UP', text='').action = 'UP'
    right.operator('mmr_kumopult_bac.list_action', icon='TRIA_DOWN', text='').action = 'DOWN'
    right.separator()
    right.operator('mmr_kumopult_bac.child_mapping', icon='CON_CHILDOF', text='')
    right.operator('mmr_kumopult_bac.name_mapping', icon='CON_TRANSFORM_CACHE', text='')
    right.operator('mmr_kumopult_bac.mirror_mapping', icon='MOD_MIRROR', text='')


class MMR_BAC_UL_mappings(bpy.types.UIList):
    def draw_item(self, context, layout, data, item, icon, active_data, active_propname, index, flt_flag):
        ...
        def mapping():
            ...
            row.prop_search(item, 'selected_owner', s.get_owner_armature(), 'bones', text='', translate=False,
                            icon='BONE_DATA')
            row.label(icon='BACK')
            row.prop_search(item, 'target', s.get_target_armature(), 'bones', text='', translate=False,
                            icon='BONE_DATA')
...
class MMR_BAC_MT_SettingMenu(bpy.types.Menu):
    bl_label = "Setting"
...
class MMR_BAC_MT_presets(bpy.types.Menu):
    bl_label = "映射表预设"
    preset_subdir = "mmr_kumopult_bac"
    preset_operator = "script.execute_preset"
    draw = bpy.types.Menu.draw_preset


class MMR_AddPresetBACMapping(AddPresetBase, bpy.types.Operator):
    bl_idname = "mmr_populist_bac.mappings_preset_add"
    bl_label = "添加预设 Add BAC Mappings Preset"
    bl_description = "将当前骨骼映射表保存为预设，以供后续直接套用"
    preset_menu = "MMR_BAC_MT_presets"
...
class MMR_BAC_OT_OpenPresetFolder(bpy.types.Operator):
    bl_idname = 'mmr_kumopult_bac.open_preset_folder'
    bl_label = '打开预设文件夹'
...
class MMR_BAC_OT_SelectEditType(bpy.types.Operator):
    bl_idname = 'mmr_kumopult_bac.select_edit_type'
    bl_label = ''
...
class MMR_BAC_OT_SelectAction(bpy.types.Operator):
    bl_idname = 'mmr_kumopult_bac.select_action'
    bl_label = '列表选择操作'
...
class MMR_BAC_OT_ListAction(bpy.types.Operator):
    bl_idname = 'mmr_kumopult_bac.list_action'
    bl_label = '列表基本操作'
...
class MMR_BAC_OT_ChildMapping(bpy.types.Operator):
    bl_idname = 'mmr_kumopult_bac.child_mapping'
    bl_label = '子级映射'
...
class MMR_BAC_OT_NameMapping(bpy.types.Operator):
    bl_idname = 'mmr_kumopult_bac.name_mapping'
    bl_label = '名称映射'
...
class MMR_BAC_OT_MirrorMapping(bpy.types.Operator):
    bl_idname = 'mmr_kumopult_bac.mirror_mapping'
    bl_label = '镜像映射'
...
class MMR_BAC_OT_Bake(bpy.types.Operator):
    bl_idname = 'mmr_kumopult_bac.bake'
    bl_label = '烘培动画'
    ...
    def execute(self, context):
        ...
        bpy.context.view_layer.objects.active = bpy.context.scene.mmr_kumopult_bac_owner
        ...
        if not a:
            # 先确保源骨架上有动作
            print('源骨架上没有动作！', '确保有动作的情况下才能自动判断烘培的帧范围')
            return {'FINISHED'}
        else:
            # 选中约束的骨骼
